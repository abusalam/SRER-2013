...
$LogC = 0;
if ((GetVal($_POST, 'UserID') != "") && (GetVal($_POST, 'UserID') != "")) {
  $QueryLogin = "Select PartMapID,UserName from `SRER_Users` where `UserID`='" . GetVal($_POST, 'UserID') . "' AND MD5(concat(`UserPass`,MD5('" . $_POST['LoginToken'] . "')))='" . $_POST['UserPass'] . "'";
  ...
  if ($rows > 0) {
    ...
    $action = "JustLoggedIn";
    $Data->do_ins_query("Update SRER_Users Set LoginCount=LoginCount+1 where `UserID`='" . GetVal($_POST, 'UserID') . "' AND MD5(concat(`UserPass`,MD5('" . $_POST['LoginToken'] . "')))='" . $_POST['UserPass'] . "'");
    $Data->do_ins_query("INSERT INTO SRER_logs (`SessionID`,`IP`,`Referrer`,`UserAgent`,`UserID`,`URL`,`Action`,`Method`,`URI`) values"
    ...
    $Data->do_ins_query("INSERT INTO SRER_logs (`SessionID`,`IP`,`Referrer`,`UserAgent`,`UserID`,`URL`,`Action`,`Method`,`URI`) values"
            . "('" . $_SESSION['ID'] . "','" . $_SERVER['REMOTE_ADDR'] . "','" . mysql_real_escape_string($_SERVER['HTTP_REFERER']) . "','" . $_SERVER['HTTP_USER_AGENT']
            . "','" . GetVal($_POST, 'UserID') . "','" . mysql_real_escape_string($_SERVER['PHP_SELF']) . "','Login: Failed','"
            . mysql_real_escape_string($_SERVER['REQUEST_METHOD']) . "','" . mysql_real_escape_string($_SERVER['REQUEST_URI'] . $_SERVER['QUERY_STRING']) . "');");
  }
}
